...
class StickyForm extends Validation {
    ...
    foreach ($formConfig as $key => &$element) {
        $element['value'] = trim($data[$key] ?? '');
        $customErrorMsg = $element['errorMsg'] ?? null;

        if (isset($element['type']) && in_array($element['type'], ['text', 'textarea', 'password']) && isset($element['regex'])) {
            if ($element['required'] && $element['value'] === '') {
                ...
                // Optional empty field — skip
            } else {
                $isValid = $this->checkFormat($element['value'], $element['regex'], $customErrorMsg);
                if (!$isValid) {
                    $element['error'] = $this->getErrors()[$element['regex']] ?? $customErrorMsg ?? "Invalid {$key}.";
                    $formConfig['masterStatus']['error'] = true;
                }
            }
        }
        ...
        $formConfig['fields']['confirm_password']['error'] = 'Passwords do not match.';
        $formConfig['masterStatus']['error'] = true;
    }
